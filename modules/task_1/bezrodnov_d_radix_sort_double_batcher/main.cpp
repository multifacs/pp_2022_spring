...
/*TEST(radix_sort_double_batcher_seq, test_sort_random_all) {
    std::vector<double> vec = get_random_double_vector(100000000);
    vec = radix_sort(vec, 3);
    long start_time = clock();
    bool is_sorting = check_sort(vec);
    long end_time = clock();
    std::cout<<"Seq time 1 000 000 = "<<end_time - start_time<<std::endl;
    ASSERT_TRUE(is_sorting);
}*/
