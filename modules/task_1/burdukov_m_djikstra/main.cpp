...
TEST(burkukov_dijkstra_sequential, create) {
  std::vector<std::vector<int>> a(1, std::vector<int>(1, 0));
  std::vector<int> d = dijkstra(0, a);
  ASSERT_EQ(d[0], 0);
}

TEST(burkukov_dijkstra_sequential, simple_path_1v) {
...
TEST(burkukov_dijkstra_sequential, simple_path_20v) {
  int size = 20;
...
TEST(burkukov_dijkstra_sequential, tree) {
  int size = 20;
  std::vector<std::vector<int>> a(size, std::vector<int>(size, 0));
  for (int i = 0; i < size; i++)
...
TEST(burkukov_dijkstra_sequential, tree_simp) {
