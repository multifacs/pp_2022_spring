...
TEST(MonteCarloIntegration, one_dims_sin) {
  ...
  double answer = MonteCarloIntegration_(integrand, intervals, 100);
  double real_answer = 1.0;

  ASSERT_NEAR(real_answer, answer, 0.5);
}

TEST(MonteCarloIntegration, one_dims_quadratic) {
  const size_t dims = 1;
  std::function<double(const std::vector<double>&)> integrand =
  ...
  double answer = MonteCarloIntegration_(integrand, intervals, N);
  double real_answer = 41.6667;

  ASSERT_NEAR(real_answer, answer, 2.0);
}

TEST(MonteCarloIntegration, two_dims_hyperboloid) {
  const size_t dims = 2;
  std::function<double(const std::vector<double>&)> integrand =
...
TEST(MonteCarloIntegration, two_dims_inverse_hyperboloid) {
  const size_t dims = 2;
  std::function<double(const std::vector<double>&)> integrand =
      [](const std::vector<double>& points) {
...
TEST(MonteCarloIntegration, three_dims_integral) {
  const size_t dims = 3;
  std::function<double(const std::vector<double>&)> integrand =
      [](const std::vector<double>& points) {
