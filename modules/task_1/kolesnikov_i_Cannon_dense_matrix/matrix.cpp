...
void Matrix::shiftLeft(std::vector< std::vector<double>> &matr, size_t pos, size_t block_count, size_t skew) {
    ...
    for (size_t i = 0; i < skew; ++i) {
        ...
        for (size_t j = 0; j < skew; ++j) {
            tmp_matr[i][j] = matr[i + skew * pos][j];
        }
    }
    for (size_t k = 0; k < block_count - 1; ++k) {
        for (size_t i = 0; i < skew; ++i) {
            for (size_t j = 0; j < skew; ++j) {
                matr[i + skew * pos][j + skew * k] = matr[i + skew * pos][j + skew * (k + 1)];
            }
        }
    }
    for (size_t i = 0; i < skew; ++i) {
        for (size_t j = 0; j < skew; ++j) {
            matr[i + skew * pos][j + skew * (block_count - 1)] = tmp_matr[i][j];
        }
    }
}
...
void Matrix::shiftUp(std::vector< std::vector<double>> &matr, size_t pos, size_t block_count, size_t skew) {
    ...
    for (size_t i = 0; i < skew; ++i) {
        ...
        tmp_matr.push_back(vec);
        for (size_t j = 0; j < skew; ++j) {
            tmp_matr[i][j] = matr[i][j + skew * pos];
        }
    }
    for (size_t i = 0; i < block_count - 1; ++i) {
        for (size_t j = 0; j < skew; ++j) {
            for (size_t k = 0; k < skew; ++k) {
                matr[j + skew * i][k + skew * pos] = matr[j + skew * (i + 1)][k + skew * pos];
            }
        }
    }
    for (size_t i = 0; i < skew; ++i) {
        for (size_t j = 0; j < skew; ++j) {
            matr[i + skew * (block_count - 1)][j + skew * pos] = tmp_matr[i][j];
        }
    }
}

void Matrix::mutiplyByBlock(std::vector< std::vector<double>> block1, std::vector< std::vector<double>> block2, std::vector< std::vector<double>> &res_block, size_t shift_l, size_t shift_r, size_t skew) {
    for (size_t i = 0; i < skew; i++)
        for (size_t j = 0; j < skew; j++)
            for (size_t k = 0; k < skew; k++)
...
std::vector< std::vector<double>> Matrix::cannonAlgorithmSeq(Matrix matrix2, std::vector< std::vector<double>> res_matrix, size_t block_size, size_t block_count) {
