		...
		for (int y = 1; y < image.size; y++) {
			if (!image.Get(x,y))
				...
				int _00 = areas.Get(x, y);
					  ...
					  int total_non_bg = (int)(_1 != -1) + (int)(_2 != -1) + (int)(_3 != -1) + (int)(_4 != -1);
					...
					for (int i = 0; i < sz; i++) {
						if (nonzero[i] != nonzero[0]) {
							if (equivalents.find(nonzero[0]) != equivalents.end()) {
								if (std::find(equivalents.at(nonzero[0]).begin(), equivalents.at(nonzero[0]).end(), nonzero[i]) == equivalents.at(nonzero[0]).end()) {
									// Not contains
									equivalents.at(nonzero[0]).push_back(nonzero[i]);
								}
							}
							else if (equivalents.find(nonzero[i]) != equivalents.end()) {
								if (std::find(equivalents.at(nonzero[i]).begin(), equivalents.at(nonzero[i]).end(), nonzero[i]) == equivalents.at(nonzero[i]).end()) {
									// Not contains
									equivalents.at(nonzero[i]).push_back(nonzero[0]);
								}
							}
							else {
								equivalents.insert(std::make_pair(nonzero[0], std::vector<int>()));
								if (std::find(equivalents.at(nonzero[0]).begin(), equivalents.at(nonzero[0]).end(), nonzero[i]) == equivalents.at(nonzero[0]).end()) {
									// Not contains
									equivalents.at(nonzero[0]).push_back(nonzero[i]);
								}
