...
TEST(TRAPEZOIDAL_RULE_MPI, TEST_DIM_1_FUNC_1) {
  ...
  seq_result = getSequentialTrapezoidal(n, limits, f);

  std::cout << "Seq. result: " << seq_result << std::fixed
...
TEST(TRAPEZOIDAL_RULE_MPI, TEST_DIM_2_FUNC_2) {
  std::vector<std::pair<double, double>> limits(2);
  limits = {{0, 2}, {0, 1}};
  int n = 200;
  double seq_result;
...
TEST(TRAPEZOIDAL_RULE_MPI, TEST_DIM_2_FUNC_3) {
  std::vector<std::pair<double, double>> limits(2);
  limits = {{0, 5}, {3, 7}};
  int n = 100;
  double seq_result;
...
TEST(TRAPEZOIDAL_RULE_MPI, TEST_DIM_3_FUNC_4) {
  std::vector<std::pair<double, double>> limits(2);
  limits = {{0, 1}, {-4, 2}, {5, 7}};
  int n = 20;
  double seq_result;
...
TEST(TRAPEZOIDAL_RULE_MPI, TEST_DIM_3_FUNC_5) {
  std::vector<std::pair<double, double>> limits(2);
