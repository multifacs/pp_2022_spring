...
class Union {
 ...
 public:
  // создать пустую структуру данных с N изолированными множествами
  Union(int N) {
  ...
  // возвращаем идентификатор компоненты, соответствующей объект index
  int get_label(int index) {
    int root = index;
    while (root != label_image[root])
      root = label_image[root];  // пока не найдем корневой индентификатор
    while (label_image[index] != root) {  // делаем некоторое сжатие пути
  ...
  // объединение компонент, содержащих my_index и neighbor_index
  void set_label(int my_index, int neighbor_index) {
    ...
    // делаем меньший идентификатор компоненты
