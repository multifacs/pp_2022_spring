...
/*TEST(MonteCarlo_test_omp, test_4) {
    std::vector<double> left = { 0.0, 0.0 };
    std::vector<double> right = { 2.0, 2.0 };
    const uint64_t steps = 45000000;
    const int thread_num = 4;

    double t1 = omp_get_wtime();
    double res_omp = MonteCarloOmp(test_func4, left, right, steps, thread_num);
    double t2 = omp_get_wtime();
    double res_seq = MonteCarloSeq(test_func4, left, right, steps * thread_num);
    double t3 = omp_get_wtime();
    std::cout << "OMP TIME: " << t2 - t1 << "\nSequential TIME: " << t3 - t2
        << "\nSpeedUp: " << (t3 - t2) / (t2 - t1) << "\n";
    ASSERT_NEAR(res_omp, res_seq, 0.5);
}*/
