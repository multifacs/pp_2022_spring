...
#include "../../../modules/task_2/poplavsky_a_bin_convex_hull/conv.h"
...
#pragma omp parallel num_threads(THREADS)
  {
    ...
    if (id == THREADS - 1) {
      local_coordinates = graham_seq(std::vector<Coordinates>(
          input_coordinates.begin() + step * id, input_coordinates.end()));
    } else {
      local_coordinates = graham_seq(std::vector<Coordinates>(
          input_coordinates.begin() + step * id,
          input_coordinates.begin() + step * (id + 1)));
    }
#pragma omp critical
    {
