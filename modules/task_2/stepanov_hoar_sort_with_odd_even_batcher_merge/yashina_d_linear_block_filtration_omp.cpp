...
#pragma omp single 
	...
	arr[omp_get_thread_num()] = std::vector<float>(vec->begin() + omp_get_thread_num()*part, vec->begin() + std::min((omp_get_thread_num() + 1)*part, static_cast<int>(vec->size())));
	hoar_sort_for_parts(&arr[omp_get_thread_num()], 0, arr[omp_get_thread_num()].size() - 1);
#pragma omp single 
	{
