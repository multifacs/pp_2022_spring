...
#include "../../../modules/task_2/zaitsev_a_matrix_cannon_block_mult_omp/matrix.h"
...
#pragma omp parallel num_threads(num_threads) \
    shared(first_multiplier, second_multiplier) private(i, j)
#pragma omp for schedule(dynamic)
  for (i = 1; i < count_of_block; ++i) {
    for (j = 0; j < i; ++j) {
      shift_to_Left(&first_multiplier, i, count_of_block, size_of_block);
      shift_to_Up(&second_multiplier, i, count_of_block, size_of_block);
    }
  }
  for (i = 0; i < count_of_block; ++i) {
#pragma omp parallel num_threads(num_threads) private(i, j, k, l) \
    shared(first_multiplier, second_multiplier, composition)
#pragma omp for schedule(static)
    for (j = 0; j < count_of_block; ++j) {
      for (k = 0; k < count_of_block; ++k) {
        multiply_to_Block(first_multiplier, second_multiplier, composition, j,
                          k, size_of_block);
      }
    }
