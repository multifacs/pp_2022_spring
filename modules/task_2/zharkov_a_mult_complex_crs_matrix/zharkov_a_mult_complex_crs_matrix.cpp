...
SparseMatrix::SparseMatrix(int _size) { size = _size; }

SparseMatrix::SparseMatrix(const SparseMatrix& a)
    : size(a.size), values(a.values), cols(a.cols), pointers(a.pointers) {}
...
std::vector<int> SparseMatrix::getCols() const { return cols; }

std::vector<int> SparseMatrix::getPointers() const { return pointers; }

int SparseMatrix::getSize() const { return size; }

void SparseMatrix::setSize(const int size) { this->size = size; }

void SparseMatrix::setValues(const std::vector<std::complex<double>>& val) {
  values = val;
}

void SparseMatrix::setCols(const std::vector<int>& col) { cols = col; }
...
SparseMatrix SparseMatrix::transposition() const {
  ...
  for (int i = 0; i < size; ++i) {
    ...
    a.pointers.push_back(a.pointers.back() +
                         static_cast<int>(helpCols[i].size()));
  }
  return a;
}
...
#pragma omp for private(used, cur) schedule(static)
  ...
  for (int i = 0; i < size; ++i) {
    std::copy(parallel_columns[i].begin(), parallel_columns[i].end(),
              std::back_inserter(ans.cols));
    std::copy(parallel_values[i].begin(), parallel_values[i].end(),
              std::back_inserter(ans.values));
    ans.pointers.emplace_back(ans.pointers.back() +
