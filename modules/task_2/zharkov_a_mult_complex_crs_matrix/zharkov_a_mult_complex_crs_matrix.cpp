...
SparseComplexMatrix::SparseComplexMatrix(
    int _rows_num, int _cols_num, std::vector<std::complex<double>> _values,
...
SparseComplexMatrix::SparseComplexMatrix(
    const SparseComplexMatrix& sparse_complex_matrix) {
...
std::vector<std::vector<std::complex<double>>> randomMatrix(int _rows_num,
  ...
  for (int i = 0; i < _rows_num; ++i) result[i].resize(_cols_num);
  for (int i = 0; i < _rows_num; ++i)
    for (int j = 0; j < _cols_num; ++j) {
      std::complex<double> val(0.0, 0.0);
      result[i][j] = val;
      if (prob(gen) <= percent) {
        std::complex<double> tmp(static_cast<double>(gen() % 10),
                                 static_cast<double>(gen() % 10));
        result[i][j] = tmp;
      }
    }
  return result;
...
SparseComplexMatrix SparseComplexMatrix::matrixToCRS(
...
SparseComplexMatrix SparseComplexMatrix::transposeCRS() {
  ...
  for (int i = 0; i < rows_num; ++i) {
    ...
    for (int j = 0; j < row_count[i]; ++j) row_idxs.push_back(i);
  }
...
bool SparseComplexMatrix::operator==(const SparseComplexMatrix& mat) const& {
  if (rows_num != mat.rows_num || cols_num != mat.cols_num ||
      values != mat.values || col_index != col_index ||
      row_index != mat.row_index) {
    return false;
  } else {
    for (unsigned i = 0; i < values.size(); ++i) {
      if (values[i].real() != mat.values[i].real() ||
          values[i].imag() != mat.values[i].imag())
        return false;
    }
    return true;
  }
...
SparseComplexMatrix SparseComplexMatrix::operator*(
...
SparseComplexMatrix SparseComplexMatrix::crsParallelMult(
    const SparseComplexMatrix& mat) const& {
  SparseComplexMatrix result(rows_num, mat.cols_num);
  SparseComplexMatrix tmp;
  tmp = mat;
  ...
  result.row_index.push_back(0);
  for (unsigned i = 1; i < row_index.size(); ++i) {
    std::vector<std::complex<double>> vals(tmp.row_index.size(),
                                           std::complex<double>(0.0, 0.0));
    std::vector<int> cols(tmp.row_index.size(), 0);
#pragma omp parallel
    {
#pragma omp for schedule(static)
      for (int j = 1; j < static_cast<int>(tmp.row_index.size()); ++j) {
        std::complex<double> s = 0;
        int iter1 = row_index[i - 1];
        int iter2 = tmp.row_index[j - 1];
        while ((iter1 < row_index[i]) && (iter2 < tmp.row_index[j])) {
...
void SparseComplexMatrix::printCRS() {
  for (unsigned i = 0; i < values.size(); ++i) std::cout << values[i] << " ";
  std::cout << "\n";
