...
#include "./monte_carlo_tbb.h"

const size_t N = 10000;

TEST(MonteCarloIntegration, tbb_one_dims_sin) {
  const size_t dims = 1;
...
TEST(MonteCarloIntegration, tbb_one_dims_quadratic) {
  const size_t dims = 1;
  std::function<double(const std::vector<double>&)> integrand =
...
TEST(MonteCarloIntegration, tbb_two_dims_hyperboloid) {
  const size_t dims = 2;
  std::function<double(const std::vector<double>&)> integrand =
...
TEST(MonteCarloIntegration, tbb_two_dims_inverse_hyperboloid) {
  const size_t dims = 2;
  std::function<double(const std::vector<double>&)> integrand =
      [](const std::vector<double>& points) {
