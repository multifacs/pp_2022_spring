...
struct DischargeSort {
    ...
    size_t byte;
	static tbb::mutex m;
    explicit DischargeSort(vector* ta, size_t _b, std::vector<int>* _list): arr(ta), byte(_b), list(_list) {}
...
void union_from_lists_par(std::vector<int> *list, vector* v, vector* res, size_t byte) {
...
void discharge_sort(vector* v, vector* res) {
    ...
    for (size_t j = 0; j < sizeof(double); j++) {
		...
		tbb::parallel_for(tbb::blocked_range<int>(0, list.size()), [&list](const tbb::blocked_range<int>& r) {
			int end = r.end();
			int begin = r.begin();
			for (int i = begin; i < end; i++) {
				list[i] = 0;
