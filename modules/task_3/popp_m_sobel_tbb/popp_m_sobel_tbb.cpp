...
image<uint8_t> sobelSequence(image<uint8_t> inImage) {
  ...
  for (size_t i = 1; i < inImage._rows - 1; ++i)
    for (size_t j = 1; j < inImage._columns - 1; ++j) {
      ...
      result._matrix[index] = sqrt(resX * resX + resY * resY) > 255
                                  ? 255
                                  : sqrt(resX * resX + resY * resY);
    }
  return result;
}
...
class sTbb {
 ...
 public:
  sTbb(const image<uint8_t>& in, image<uint8_t>* res)
      : input(in), result(res) {}
    ...
    for (int i = rows.begin(); i != rows.end(); ++i) {
      for (int j = 1; j < static_cast<int>(input._columns) - 1; ++j) {
        ...
        for (int ki = -1; ki <= 1; ki++)
          for (int kj = -1; kj <= 1; kj++) {
            resX += KERNEL_X._matrix[indexKernel] *
                    input._matrix[(i + ki) * input._columns + j + kj];
            resY += KERNEL_Y._matrix[indexKernel] *
                    input._matrix[(i + ki) * input._columns + j + kj];
            indexKernel++;
          }
