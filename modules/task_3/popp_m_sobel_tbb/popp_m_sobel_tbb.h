...
struct image {
  ...
  size_t _columns;
  size_t _rows;
  image(std::vector<T> matrix, size_t columns, size_t rows)
      : _matrix(matrix), _columns(columns), _rows(rows) {
    if (matrix.size() != rows * columns) throw -1;
  }

  image(size_t columns, size_t rows) : _columns(columns), _rows(rows) {
    if (rows <= 0 || columns <= 0) {
      throw -1;
    }
  ...
  bool operator==(const image& img) const {
    return this->_matrix == img._matrix && this->_rows == img._rows &&
...
#endif  //  MODULES_TASK_3_POPP_M_SOBEL_TBB_POPP_M_SOBEL_TBB_H_
