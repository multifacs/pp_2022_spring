...
#include "../../modules/task_3/zharkov_a_mult_crs/zharkov_a_mult_crs.h"
#include "tbb/tbb.h"
  ...
  for (int i = 0; i < first.rows; ++i)
    for (int j = 0; j < second.cols; ++j) {
      ...
      for (int k = 0; k < first.cols; ++k)
        res = res +
              first.val[first.cols * i + k] * second.val[second.cols * k + j];
...
MatrixCRS matrixCRSMult(const MatrixCRS& first, const MatrixCRS& second_a) {
  ...
  if (first.ptrs.size() != second.ptrs.size())
    throw std::runtime_error("Error of size");
...
class ParallelProcessing {
 public:
  ...
  ParallelProcessing(const MatrixCRS& first, const MatrixCRS& second,
                     std::vector<int>* tmpResultPtrs,
                     std::vector<int>* tmpResultColsPose,
                     std::vector<std::complex<int>>* tmpResultValue)
      : first(first),
...
MatrixCRS matrixCRSMultTBB(const MatrixCRS& first, const MatrixCRS& second_a) {
  ...
  ParallelProcessing pp(first, second, &tmpResultPtrs, tmpResultColsPose,
                        tmpResultValue);
...
void print(const MatrixCRS& in) {
  ...
  for (size_t i = 0; i < in.val.size(); i++) std::cout << in.val[i] << ' ';
  std::cout << std::endl;
  ...
  for (size_t i = 0; i < in.ptrs.size(); i++) std::cout << in.ptrs[i] << ' ';
  std::cout << std::endl;
}
