...
TEST(MatrixComplex_STD, get_works) {
    MatrixComplex matrix(3);
    EXPECT_NO_THROW(matrix.get(1, 1));
}

TEST(MatrixComplex_STD, get_throws_exception) {
    MatrixComplex matrix(3);
    EXPECT_ANY_THROW(matrix.get(4, 4));
}

TEST(MatrixComplex_STD, multiplication_works) {
...
TEST(Matrix_Multiplication_STD, parallel_multiplication_returns_same_result) {
    MatrixComplex matrix1(4);
    MatrixComplex matrix2(4);
    ...
    std::cout << "Parallel time: " << search_time*2 << std::endl;*/
