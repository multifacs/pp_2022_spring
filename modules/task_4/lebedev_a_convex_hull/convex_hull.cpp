...
size_t get_effective_num_threads(size_t size) {
    ...
    return std::min<int>(static_cast<int>(size / MIN_STEP + add), std::thread::hardware_concurrency());
}


void lab4::convex_hull(const cv::Mat& input, std::vector<cv::Point2d>* output, Version v) {
    std::vector<cv::Point2d> non_zeros;
    cv::findNonZero(input, non_zeros);
    lab4::convex_hull(&non_zeros, output, v);
}

void lab4::convex_hull(std::vector<cv::Point2d>* input, std::vector<cv::Point2d>* output, Version v) {
    if (v != SEQUENTIAL) {
        ...
        std::vector<std::thread> threads(jobs - 1);
        for (size_t i = 0; i < jobs - 1; ++i) {
            threads[i] = std::thread([i, &input, &hulls, &step]() {
                convex_hull_impl(input->begin() + (i + 1) * step, input->begin() + (i + 2) * step, &hulls[i + 1]);
                });
        }
        convex_hull_impl(input->begin(), input->begin() + step, &hulls[0]);
        for (auto& t : threads) {
            t.join();
        }
        ...
        std::vector<cv::Point2d> concat(size + last.size());
