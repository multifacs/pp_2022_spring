...
std::vector<double> hoarBatcherPar(const std::vector<double>& vec, int size) {
    ...
    auto t1 = std::async(hoarSort, &vec1, 0, vec1.size() - 1);
    auto t2 = std::async(hoarSort, &vec2, 0, vec2.size() - 1);
    t1.wait();
    t2.wait();

    auto t3 = std::async(evenBatcherSort, vec1, vec2);
    auto t4 = std::async(oddBatcherSort, vec1, vec2);
    even = t3.get();
    odd = t4.get();
