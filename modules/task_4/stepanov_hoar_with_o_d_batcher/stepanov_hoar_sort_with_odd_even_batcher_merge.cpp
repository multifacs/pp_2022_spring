...
void hoar_sort_with_batch_merge_std(std::vector<float>* vec) {
	...
	for (size_t i = 0; i < nthreads; i++) {
		threads[i] = std::thread([&](const int i) {
			arr[i] = std::vector<float>(vec->begin() + (i*part), vec->begin() + std::min((i + 1)*part, vec->size()));
			hoar_sort_for_parts(&arr[i],0, arr[i].size()-1);
		}, i);
	}
	...
	while (parts_count > 1) {
		for (int i = 0; i < (parts_count / 2); i++) {
			threads[i] = std::thread(batch_merge, &arr[i], &arr[parts_count - 1 - i], &arr[i]);
		}
		for (size_t i = 0; i < (parts_count / 2); i++) threads[i].join();
