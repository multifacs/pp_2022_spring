...
#include "../../modules/task_4/zaitseva_k_jarvis/jarvis.h"
...
std::vector<Point> Jarvis_parallel_std(const std::vector<Point>& points) {
  ...
  int th_num = 0;
  auto func = [&shells](const std::vector<Point>& points, int idx) {
    shells[idx] = Jarvis(points);
  };
  for (auto it = std::begin(threads); it != std::end(threads); ++it) {
    ...
    *it = std::thread(
        func, std::vector<Point>(points.begin() + th_num * step, end), th_num);
    th_num++;
  }
  for (auto&& i : threads) {
    i.join();
